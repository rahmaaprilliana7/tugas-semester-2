...
def halaman_riwayat(anggaran: AnggaranHarian): #sudah di upgrade
    ...
    with st.spinner("Memuat riwayat..."): 
        # Mengambil data dengan kolom ID agar pengguna bisa melihat ID Transaksi yang ingin dihapus
        # Catatan: Pastikan query database kamu mengembalikan kolom 'id' ke DataFrame jika ingin menampilkannya
    ...
    if df_transaksi is None: 
        ...
        # Input komponen form untuk memilih ID transaksi yang akan dihapus
        id_hapus = st.number_input("Masukkan ID Transaksi yang ingin dihapus:", min_value=1, step=1, key="input_id_hapus")
        tombol_hapus = st.button("Hapus Transaksi Terpilih", type="primary")
        
        if tombol_hapus:
            # Memasukkan status pemicu konfirmasi ke dalam session state Streamlit
            st.session_state.pemicu_konfirmasi = True

        # Logika Alur Konfirmasi Penghapusan Data
        if st.session_state.get("pemicu_konfirmasi", False):
            st.warning(f"Apakah Anda yakin ingin menghapus transaksi dengan ID {id_hapus}?", icon="⚠️")
            
            kol_konfirmasi1, kol_konfirmasi2 = st.columns([1, 6])
            with kol_konfirmasi1:
                tombol_ya = st.button("Ya, Hapus", type="danger")
            with kol_konfirmasi2:
                tombol_batal = st.button("Batal")
                
            if tombol_ya:
                with st.spinner("Sedang menghapus data..."):
                    sukses = anggaran.hapus_transaksi(int(id_hapus))
                    if sukses:
                        st.success(f"Transaksi dengan ID {id_hapus} berhasil dihapus!", icon="✅")
                        # Menghapus flag pemicu dan membersihkan cache agar visualisasi data langsung terupdate
                        st.session_state.pemicu_konfirmasi = False
                        st.cache_data.clear()
                        st.rerun()
                    else:
                        st.error("Gagal menghapus transaksi. Pastikan ID tersebut valid dan tersedia.", icon="❌")
                        st.session_state.pemicu_konfirmasi = False
...
def halaman_ringkasan(anggaran: AnggaranHarian):
    ...
    with col_filter1:
        ...
        elif pilihan_periode == "Pilih Tanggal":  # Disamakan dengan opsi selectbox
