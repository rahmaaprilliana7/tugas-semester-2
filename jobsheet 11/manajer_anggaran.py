...
class AnggaranHarian:
    ...
    def get_dataframe_transaksi(self, filter_tanggal: datetime.date | None = None) -> pd.DataFrame:
        # 1. Pastikan kolom 'id' ikut dipanggil di query SQL
        ...
        if not df.empty:
            try:
                ...
                # Perbaikan: Menghapus titik yang nyasar di ujung baris ini
                df['Jumlah (Rp)'] = df['jumlah'].map(
                    lambda x: f"Rp {x or 0:,.0f}".replace(",", ".")
                )
                
            # 2. Susunan kolom yang sudah kamu buat (Sudah Benar)
...
#Perubahan Bagian Backend

    def hapus_transaksi(self, id_transaksi: int) -> bool:
        """Menghapus transaksi berdasarkan ID yang diberikan."""
        sql = "DELETE FROM transaksi WHERE id = ?"
        params = (id_transaksi,)
        
        # database.execute_query akan mengembalikan lastrowid (bukan None) jika berhasil dijalankan
